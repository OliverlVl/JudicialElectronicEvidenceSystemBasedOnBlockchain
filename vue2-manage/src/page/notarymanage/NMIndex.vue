      ...
      try {
        ...
        await autmanQuery(autQuery).then((result) => {
          if (result.status) {
            ...
            console.log("获取机构ID失败");
          }
        });
        //统计生成
        await orgStasGen().then((result) => {
          if (result.status) {
            console.log("机构统计生成成功");
          } else {
            console.log("机构统计生成失败");
          }
        });
        await orgStasQue(query).then((result) => {
          if (result.status) {
            ...
            this.orgInfo = {};           
            result.data.forEach((item) => {
              if (item.data.organizationId == this.orgID) {
                this.orgInfo = item.data;
              }
            });*/
          } else {
            console.log("机构统计查询失败");
          }
        });
      } catch (error) {
        throw new Error(error.message);
